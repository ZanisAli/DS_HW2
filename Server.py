...
class SudokuServer():
    ...
    def turn(self, parms):
        ...
        col = session.visible_numbers[player][x * cells: (x+1) * cells]

        # check same number in column
        row = session.visible_numbers[player][y::cells]

        # check same number small rect
        sy = y - y % 3
        sx = x - x % 3
        rect = session.visible_numbers[player][sy + cells * sx:sy + 3 + cells * sx] + \
               session.visible_numbers[player][sy + cells * (sx+1):sy + 3 + cells * (sx+1)] + \
               session.visible_numbers[player][sy + cells * (sx+2):sy + 3 + cells * (sx+2)]

        if session.visible_numbers[player][y + cells * x] == 0:
            session.scores[player] -= 1
            if int(n) not in row and int(n) not in col and int(n) not in rect:
